...
const rawVisibleReferencePattern = /(docs\/|apps\/|clients\/|[A-Z]:\\|\.md\b|\.json\b|\.tsx?\b)/i;
...
const personaSlideCount = parsed.slides.filter((slide) => slide.section === 'personas').length;
const chapterDividerCount = parsed.slides.filter(
  (slide) => slide.section === 'chapter-divider' || slide.layout === 'chapter',
).length;
const slidesMissingArabicBody = [];
const slidesWithUnsafeSourceLabels = [];
const slidesWithRawVisibleReferences = [];
...
parsed.slides.forEach((slide, index) => {
  ...
  if (visibleBodyText && rawVisibleReferencePattern.test(visibleBodyText)) {
    slidesWithRawVisibleReferences.push(slide.id || index + 1);
  }

  if (arabicFirstDeck && visibleBodyText && !/\p{Script=Arabic}/u.test(visibleBodyText)) {
...
if (slidesWithRawVisibleReferences.length > 0) {
  fail(`visible slide content cannot expose raw internal repo paths or filenames. Offending slides: ${slidesWithRawVisibleReferences.join(', ')}`);
}
...
if (
  configuredProject &&
  hasSubstantiveContent(directStrategyPath) &&
  /Secondary Persona/i.test(readUtf8(directStrategyPath)) &&
  personaSlideCount < 2
) {
  fail(`expected at least 2 persona slides when the strategy includes a secondary persona, found ${personaSlideCount}`);
}
...
if (configuredProject && parsed.slides.length >= 20 && chapterDividerCount < 3) {
  fail(`expected at least 3 chapter-divider slides in a full client deck, found ${chapterDividerCount}`);
}

if (
  configuredProject &&
  hasSubstantiveContent(directStrategyPath) &&
  hasSubstantiveContent(competitorScanPath) &&
  hasSubstantiveContent(adCopyPath) &&
  hasSubstantiveContent(campaignFlowPath) &&
  parsed.slides.length < 30
) {
  fail(`full direct-response client deck is too shallow: expected at least 30 slides, found ${parsed.slides.length}`);
}
